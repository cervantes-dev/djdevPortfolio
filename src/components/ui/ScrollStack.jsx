...
// ── itemHeight prop added (default keeps original 400px / h-100 behaviour) ──
export const ScrollStackItem = ({ children, itemClassName = '', itemHeight = '420px' }) => (
  <div
...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback((overrideScrollTop) => {
    if (!cardsRef.current.length || isUpdatingRef.current) return;
    isUpdatingRef.current = true;

    const { scrollTop: rawScrollTop, containerHeight } = getScrollData();
    const scrollTop = overrideScrollTop ?? rawScrollTop;
  ...
  useLayoutEffect(() => {
    ...
    // ── Wheel passthrough ─────────────────────────────────────────────────────
    // When the internal scroller hits its very bottom (or top), stop trapping
    // wheel events and forward them to the window so the page keeps scrolling.
    const handleWheel = (e) => {
  ...
  const containerStyles = useWindowScroll
    ...
    : { WebkitOverflowScrolling: 'touch', WebkitTransform: 'translateZ(0)', transform: 'translateZ(0)', willChange: 'scroll-position' };
